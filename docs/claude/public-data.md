...
### ✅ 수행 기록 (2026-07-29 전국 완료)
| 범위 | CSV 대상 | 좌표변환 성공 | INSERT |
|------|---------|--------------|--------|
| 예산군 | 84 | 84 (100%) | 84 |
| 충청남도 | 2,640 | 2,395 (91%) | 2,395 |
| **전국 나머지** | 50,754 | 49,478 (97.5%) | 49,477 |
| **누적 toilet** | | | **51,994건** |

- 전국 지오코딩 소요 **약 95분** (건당 ~0.11s). 카카오 API 한도 초과나 429는 발생하지 않음.
- 실패 1,276건(2.5%)은 주소가 불완전하거나(`신방쉼터A 화장실` 등) 카카오가 못 찾은 경우 — 스킵.
- 검증: 한반도 범위 밖 좌표 **0건**(기존 해외 화장실 8건은 의도된 데이터), 고도 이상치 0건,
  전량 `color=purple`/`model=cube` 일관, 사용자 업로드 101건 무변경.

⚠️ **DB 구성이 바뀌었다** — 전체 84,370건 중 화장실이 **62%**. 앱은 반경 내 가까운 순으로
16~20개만 스폰하므로 성능 문제는 없으나, **도심에서는 화면 오브젝트 상당수가 화장실**이 된다.
부담되면 ①toilet 토글 기본 꺼짐 ②지역별 솎아내기로 조정할 것.
이번 작업분만 되돌리려면 `WHERE category='toilet' AND username='DATA.GO.KR'` (51,959건).

#### 대량 작업 시 스크립트 요건 (5만 건 규모에서 실증)
- **체크포인트 필수** — 1,000건마다 JSON 저장, 재실행 시 이어서. 95분 작업이 중단돼도 손실 0.
- **429 백오프** — HTTPError 429면 20초 대기 후 재시도.
- **중복검사는 격자 인덱스로** — 5만 × 2.5천 전수 비교는 너무 느리다.
  좌표를 소수점 3자리로 반올림해 셀 키를 만들고 인접 9칸만 비교할 것.
- **INSERT는 2,000건마다 커밋** — 한 번에 몰면 트랜잭션이 길어진다.
