...
### 🔴 미해결 — 중복 등록 397쌍
같은 장소가 이름만 다르게 두 번 등록돼 있다(이름 정규화 동일 + 5km 이내 기준).
- `종로경찰서`(878) ↔ `서울종로경찰서`(1855) — 208m
- `예산경찰서`(257, WIKIMEDIA) ↔ `충남 예산경찰서`(9985, GOV.KR) — 10m
- `대흥면행정복지센터`(671, YESAN.GO.KR) ↔ `예산군 대흥면 행정복지센터`(4163, GOV.KR)

서로 다른 시기·출처(`POLICE.GO.KR`/`GOV.KR`/`WIKIMEDIA`)로 넣으며 생긴 것으로 보인다.
AR에서 같은 건물에 오브젝트가 2개 뜬다. **삭제는 되돌리기 어려우므로 대표님 승인 후 진행**할 것.
정리 시 좌표 정밀도가 높은 쪽을 남기고, 삭제는 위 "삭제 시 FK 순서"를 따를 것.
