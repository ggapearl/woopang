...
def _send_apns_to_endpoint(device_token, title, body, environment, data=None):
    """APNs HTTP/2 전송 내부 함수 (단일 환경)"""
    jwt_token = create_apns_jwt_token()
    if not jwt_token:
        safe_print(f"[APNS_DEBUG] JWT 토큰 생성 실패")
        return False, "JWT 토큰 생성 실패"
    ...
    if response.status_code == 200:
        safe_print(f"[APNS_DEBUG] {environment} 전송 성공: {device_token[:20]}...")
        return True, "success"
    else:
        response_body = ""
        try:
            ...
            pass
        safe_print(f"[APNS_DEBUG] {environment} 전송 실패: status={response.status_code}, body={response_body}, token={device_token[:20]}...")
        return False, f"status={response.status_code}, body={response_body}"


def send_apns_notification_http2(device_token, title, body, environment=None, data=None):
    """
    APNs HTTP/2 푸시 전송 (development 실패 시 production fallback)
    """
    try:
        if environment is None or not isinstance(environment, str):
            environment = "development"

        safe_print(f"[APNS_DEBUG] 전송 시작: env={environment}, token={device_token[:20]}..., title={title}")

        success, detail = _send_apns_to_endpoint(device_token, title, body, environment, data)

        if success:
            return True

        # development 실패 시 production으로 재시도 (또는 그 반대)
        fallback_env = "production" if environment == "development" else "development"
        safe_print(f"[APNS_DEBUG] {environment} 실패 ({detail}), {fallback_env}으로 재시도...")

        fallback_success, fallback_detail = _send_apns_to_endpoint(device_token, title, body, fallback_env, data)
        if fallback_success:
            # 자동으로 APNS_ENV를 학습하여 다음부터 fallback 없이 바로 전송
            global APNS_ENV
            if APNS_ENV != fallback_env:
                safe_print(f"[APNS_DEBUG] APNS_ENV 자동 변경: {APNS_ENV} → {fallback_env}")
                APNS_ENV = fallback_env
            return True
        else:
            safe_print(f"[APNS_DEBUG] {fallback_env} fallback도 실패: {fallback_detail}")
            # BadDeviceToken 또는 Unregistered인 경우 DB에서 토큰 비활성화
            if "BadDeviceToken" in fallback_detail or "Unregistered" in fallback_detail:
                try:
                    conn = get_db_connection()
                    cursor = conn.cursor()
                    cursor.execute("UPDATE device_tokens SET is_active = FALSE WHERE token = %s", (device_token,))
                    conn.commit()
                    cursor.close()
                    conn.close()
                    safe_print(f"[APNS_DEBUG] 무효 토큰 비활성화: {device_token[:20]}...")
                except Exception as db_ex:
                    safe_print(f"[APNS_DEBUG] 토큰 비활성화 실패: {db_ex}")
            return False

    except Exception as e:
        safe_print(f"[APNS_DEBUG] APNs 예외: {str(e)}")
        import traceback
        safe_print(f"[APNS_DEBUG] 스택트레이스: {traceback.format_exc()}")
        return False

def send_social_notification(to_user_id, from_username, notification_type):
    """
    팔로우/좋아요 알림 전송 (Android FCM + iOS APNs)
    ...
    try:
        ...
        # tokens 테이블에서 user_id로 토큰 조회 (최신 우선)
        cursor.execute("""
        ...
        if not token_row:
            safe_print(f"[SOCIAL_NOTIFY] user_id={to_user_id}의 토큰 없음")
            return False
        ...
        if not fcm_token and not apns_token:
            safe_print(f"[SOCIAL_NOTIFY] user_id={to_user_id}의 푸시 토큰 없음 (fcm=None, apns=None)")
            return False
        ...
        notification_data = {"notification_type": notification_type, "from_username": from_username}
        success = False

        safe_print(f"[SOCIAL_NOTIFY] 전송 시작: to={to_user_id}, type={notification_type}, platform={platform}, fcm={bool(fcm_token)}, apns={bool(apns_token)}")

        # Android FCM 알림
        if fcm_token:
            try:
                fcm_success = send_fcm_notification(fcm_token, title, body, notification_data)
                if fcm_success:
                    success = True
                    safe_print(f"[SOCIAL_NOTIFY] FCM 전송 성공: user_id={to_user_id}")
                else:
                    safe_print(f"[SOCIAL_NOTIFY] FCM 전송 실패: user_id={to_user_id}")
            except Exception as e:
                safe_print(f"[SOCIAL_NOTIFY] FCM 전송 예외: {e}")
        ...
        if apns_token:
            try:
                ...
                if apns_success:
                    success = True
                    safe_print(f"[SOCIAL_NOTIFY] APNs 전송 성공: user_id={to_user_id}")
                else:
                    safe_print(f"[SOCIAL_NOTIFY] APNs 전송 실패: user_id={to_user_id}")
            except Exception as e:
                safe_print(f"[SOCIAL_NOTIFY] APNs 전송 예외: {e}")
...
def send_fcm_notification(fcm_token, title, body, data=None):
    ...
    try:
        if not firebase_admin._apps:
            safe_print("[FCM_DEBUG] Firebase가 초기화되지 않았습니다")
            return False

        safe_print(f"[FCM_DEBUG] 전송 시작: token={fcm_token[:20]}..., title={title}")
        ...
        message = messaging.Message(
            ...
            android=messaging.AndroidConfig(
                priority='high',
                notification=messaging.AndroidNotification(channel_id='woopang_channel_high',
                    title=title,
                    body=body,
                    sound='default',
                    click_action='OPEN_MESSAGE_PANEL'
        ...
        safe_print(f"[FCM_DEBUG] 전송 성공: response={response}, token={fcm_token[:20]}...")
        return True

    except messaging.UnregisteredError:
        safe_print(f"[FCM_DEBUG] 토큰 만료/해제됨 (UnregisteredError): token={fcm_token[:20]}...")
        return False
    except messaging.InvalidArgumentError as e:
        safe_print(f"[FCM_DEBUG] 잘못된 인자 (InvalidArgumentError): {e}, token={fcm_token[:20]}...")
        return False
    except Exception as e:
        safe_print(f"[FCM_DEBUG] 전송 실패: type={type(e).__name__}, error={e}, token={fcm_token[:20]}...")
        return False
...
def register_apns_token():
    try:
        ...
        safe_print(f"[APNS_REG] iOS APNs 토큰 등록: device_id={device_id[:20]}..., user_id={user_id}")
    ...
    if request.method == 'POST':
        try:
            ...
            if all([target_lat, target_lon, radius]):
                        ...
                        message = messaging.Message(
                            ...
                            android=messaging.AndroidConfig(
                                ...
                                notification=messaging.AndroidNotification(
                                    channel_id='woopang_channel_high',
                                    title=title,
                                    body=body,
                                    sound='default'
                                )
                ...
                # DB에 관리자 메시지 저장 (앱 포그라운드 복귀 시 조회용)
                try:
                    conn = get_db_connection()
                    cursor = conn.cursor()
                    cursor.execute("""
                        INSERT INTO admin_broadcasts (title, content, sender_name, latitude, longitude, radius)
                        VALUES (%s, %s, 'WOOPANG', %s, %s, %s)
                    """, (title, body, target_lat, target_lon, radius))
                    conn.commit()
                    cursor.close()
                    conn.close()
                except Exception as db_err:
                    safe_print(f"[Warning] 관리자 메시지 DB 저장 실패: {db_err}")
                ...
                # 전체 사용자 메시지도 DB에 저장 (포그라운드 복구용)
                try:
                    conn = get_db_connection()
                    cursor = conn.cursor()
                    cursor.execute("""
                        INSERT INTO admin_broadcasts (title, content, sender_name)
                        VALUES (%s, %s, 'WOOPANG')
                    """, (title, body))
                    conn.commit()
                    cursor.close()
                    ...
                    safe_print(f"[Error] 전체 사용자 메시지 DB 저장 실패: {db_e}")
...
def test_system_message():
    ...
    try:
        ...
        # 해당 사용자의 토큰 조회 (tokens 테이블)
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("""
            SELECT fcm_token, apns_token, platform, device_id
            FROM tokens
            WHERE user_id = %s AND (fcm_token IS NOT NULL OR apns_token IS NOT NULL)
        ...
        if not token_rows:
            return jsonify({'success': False, 'error': f'user_id {user_id}의 푸시 토큰 없음'}), 404
        ...
        for row in token_rows:
            fcm_token, apns_token, platform, device_id = row
            ...
            if platform == 'android' and fcm_token:
                try:
                    ...
                    if fcm_success:
                        android_success += 1
                        safe_print(f"[Test] Android 시스템 메시지 전송 성공: device={device_id[:20]}...")
                except Exception as e:
                    safe_print(f"[Error] Android 시스템 메시지 전송 실패: {e}")

            if platform == 'ios' and apns_token:
                try:
                    apns_success = send_apns_notification_http2(apns_token, title, body, APNS_ENV, notification_data)
                    if apns_success:
                        ios_success += 1
                        safe_print(f"[Test] iOS 시스템 메시지 전송 성공: device={device_id[:20]}...")
                except Exception as e:
                    safe_print(f"[Error] iOS 시스템 메시지 전송 실패: {e}")
...
@app.route('/api/admin-messages/recent', methods=['GET'])
def get_recent_admin_messages():
    """관리자 메시지 조회 (채팅방 표시 + 포그라운드 복구 겸용)"""
    try:
        ...
        if since:
            cursor.execute("""
                SELECT id, title, content, sender_name, latitude, longitude, radius, created_at
                FROM admin_broadcasts
        ...
        messages = []
        for row in rows:
            msg_id, msg_title, msg_content, sender, msg_lat, msg_lon, msg_radius, created = row
            # 좌표 기반 필터링 (lat/lon이 있으면 반경 내 메시지만)
            if lat is not None and lon is not None and msg_lat and msg_lon and msg_radius:
                dist = calculate_distance(lat, lon, float(msg_lat), float(msg_lon))
                if dist > float(msg_radius):
                    continue
            ...
            messages.append({
                ...
                'body': msg_content,
                'sender_id': '3',
                'sender_username': sender or 'WOOPANG',
                'created_at': created.strftime('%Y-%m-%d %H:%M:%S') if created else '',
                'latitude': float(msg_lat) if msg_lat else 0,
                'longitude': float(msg_lon) if msg_lon else 0,
                'radius': float(msg_radius) if msg_radius else 0
            })

        return jsonify({'success': True, 'messages': messages})

    except Exception as e:
        safe_print(f"[Error] 관리자 메시지 조회 실패: {e}")
...
def debug_push_status():
    """
    푸시 알림 디버깅 API - 특정 유저 또는 전체 토큰 상태 조회
    GET /api/debug/push-status?user_id=6
    GET /api/debug/push-status?device_id=abc123
    GET /api/debug/push-status  (전체 요약)
    """
    try:
        user_id = request.args.get('user_id')
        ...
        result = {
            'apns_env': APNS_ENV,
            'apns_key_id': APNS_KEY_ID[:4] + '...' if APNS_KEY_ID else None,
            'apns_team_id': APNS_TEAM_ID[:4] + '...' if APNS_TEAM_ID else None,
            'apns_bundle_id': APNS_BUNDLE_ID,
            'apns_key_file_exists': os.path.exists(APNS_KEY_FILE) if APNS_KEY_FILE else False,
            'firebase_initialized': bool(firebase_admin._apps),
        }

        if user_id:
            cursor.execute("""
                SELECT device_id, platform, fcm_token, apns_token, user_id,
            ...
            result['token_count'] = len(rows)
            result['tokens'] = []
            for row in rows:
                result['tokens'].append({
                    'device_id': row[0][:20] + '...' if row[0] else None,
                    'platform': row[1],
                    'has_fcm_token': bool(row[2]),
                    'fcm_token_preview': row[2][:20] + '...' if row[2] else None,
                    'has_apns_token': bool(row[3]),
                    'apns_token_preview': row[3][:20] + '...' if row[3] else None,
                    'user_id': row[4],
                    'device_name': row[5],
                    'device_model': row[6],
                    'os_version': row[7],
            ...
            # JWT 토큰 생성 테스트
            try:
                test_jwt = create_apns_jwt_token()
                result['apns_jwt_generation'] = 'success' if test_jwt else 'failed'
            except Exception as jwt_e:
                result['apns_jwt_generation'] = f'error: {str(jwt_e)}'

        elif device_id:
            cursor.execute("""
                SELECT device_id, platform, fcm_token, apns_token, user_id,
                       device_name, device_model, os_version
                FROM tokens WHERE device_id = %s
            """, (device_id,))
            row = cursor.fetchone()
            if row:
                result['token'] = {
            ...
            cursor.execute("""
                SELECT
                    ...
                    COUNT(CASE WHEN fcm_token IS NOT NULL THEN 1 END) as has_fcm,
                    COUNT(CASE WHEN apns_token IS NOT NULL THEN 1 END) as has_apns,
            ...
            result['summary'] = {
                'total_tokens': summary[0],
                'android': summary[1],
                'ios': summary[2],
                'has_fcm_token': summary[3],
                'has_apns_token': summary[4],
                'has_user_id': summary[5],
                'no_user_id': summary[6],
            }
        ...
        safe_print(f"[Error] debug push status: {e}")
        return jsonify({'error': str(e)}), 500
...
def debug_test_push():
    ...
    try:
        data = request.get_json()
        user_id = data.get('user_id')
        target_platform = data.get('platform', 'both')
        title = data.get('title', '[DEBUG] WOOPANG 푸시 테스트')
        body = data.get('body', '이 알림이 보이면 푸시가 정상 동작합니다')
        ...
        if not rows:
            return jsonify({'error': f'user_id={user_id}의 토큰 없음', 'hint': 'GET /api/debug/push-status 로 확인'}), 404

        results = []
        for row in rows:
            fcm_token, apns_token, platform, device_id = row
            device_result = {'device_id': device_id[:20] + '...', 'platform': platform}

            if (target_platform in ['android', 'both']) and fcm_token:
                try:
                    ...
                    device_result['fcm'] = 'success' if success else 'failed'
                except Exception as e:
                    device_result['fcm'] = f'error: {str(e)}'

            if (target_platform in ['ios', 'both']) and apns_token:
                try:
                    success = send_apns_notification_http2(apns_token, title, body, APNS_ENV, {
                        'debug': 'true',
                        'notification_type': 'debug_test',
                        'timestamp': str(int(time.time()))
                    })
                    device_result['apns'] = 'success' if success else 'failed'
                except Exception as e:
                    device_result['apns'] = f'error: {str(e)}'
        ...
        safe_print(f"[Error] debug test push: {e}")
        return jsonify({'error': str(e)}), 500
    ...
    try:
        ...
        if is_port_in_use(p2p_port):
            ...
            if os.path.exists(p2p_script):
                ...
                safe_print(f"[Info] P2P 서버 백그라운드 시작 (PID: {p2p_process.pid}, 포트: {p2p_port})")
            else:
