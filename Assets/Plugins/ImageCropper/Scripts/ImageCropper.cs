...
public class ImageCropper : MonoBehaviour
{
    ...
    public static ImageCropper Instance
    {
        get
        {
            if (m_instance == null)
            {
                var prefab = Resources.Load<ImageCropper>("ImageCropper");
                if (prefab != null)
...
#pragma warning restore 0649
    ...
    private void Awake()
    {
        ...
        cropRenderCanvas.gameObject.layer = 22;
    ...
    /// <summary>
    /// SafeArea를 고려하여 Buttons 영역 위치 조정 (노치/상태바 피하기)
    /// </summary>
    private void ApplySafeAreaToButtons()
    {
        Transform buttonsTransform = transform.Find("Buttons");
        if (buttonsTransform == null) return;

        RectTransform buttonsRect = buttonsTransform as RectTransform;
        if (buttonsRect == null) return;
        ...
        if (canvas != null)
        {
            ...
            if (scaler != null && scaler.uiScaleMode == CanvasScaler.ScaleMode.ScaleWithScreenSize)
                canvasScale = canvas.GetComponent<RectTransform>().rect.height / screenHeight;
        }

        float adjustedTopInset = topInset * canvasScale;
        buttonsRect.anchoredPosition = new Vector2(0, -adjustedTopInset);
    }
    ...
    public void Show(Texture image, CropResult onCrop, Settings settings = null, ImageResizePolicy croppedImageResizePolicy = null)
    {
        ...
        // Canvas sortingOrder를 높게 설정하여 다른 UI 위에 표시
        if (canvas != null)
            canvas.sortingOrder = 32000;

        // SafeArea를 고려하여 Buttons 위치 조정 (노치/상태바 영역 피하기)
        ApplySafeAreaToButtons();
    ...
    public void ResetView(bool frameSelection)
    {
        ...
        // viewportSize가 아직 설정되지 않았으면 다음 프레임에 다시 시도
        if (m_viewportSize.x <= 0f || m_viewportSize.y <= 0f)
        {
            shouldRefreshViewport = true;
            return;
        }
