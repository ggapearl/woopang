...
{
    private static BusStationManager instance;
    public static BusStationManager Instance
    {
        get
        {
            if (instance == null)
            {
                instance = Object.FindFirstObjectByType<BusStationManager>();
            }
            return instance;
        }
    }
    ...
    private IEnumerator FetchFacilityData(float latitude, float longitude, float radius)
    {
        ...
        using (UnityWebRequest request = UnityWebRequest.Get(url))
        {
            ...
            {
                Debug.LogError($"[BusStationManager] API request failed: {request.error}");
            }
        }
    }
    ...
    private IEnumerator ProcessFacilityData(string json, float latitude, float longitude)
    {
        ...
        {
            Debug.LogError($"[BusStationManager] JSON parsing failed: {ex.Message}");
            yield break;
        }
