...
/// <summary>
/// ModelUploadManager의 업로드 페이지에 CategoryToggle UI 오브젝트를 자동 생성/연결
/// PetFriendlyToggle 위에 동일한 레이아웃으로 배치
/// </summary>
[InitializeOnLoad]
public class ModelUploadCategoryToggleSetup
{
    // PetFriendlyToggle과 SeparateRestroomsToggle 사이 간격
    private const float TOGGLE_SPACING = 73f;

    static ModelUploadCategoryToggleSetup()
    {
    ...
    {
        ModelUploadManager mgr = Object.FindFirstObjectByType<ModelUploadManager>(FindObjectsInactive.Include);
        if (mgr == null) return;
