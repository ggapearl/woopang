...
/// <summary>
/// CubeDataFixManager의 Fix UI 페이지에 CategoryToggle UI 오브젝트를 자동 생성/연결
/// CubeUploadCategoryToggleSetup과 동일한 패턴
/// </summary>
[InitializeOnLoad]
public class CubeDataFixCategoryToggleSetup
{
    private const float TOGGLE_SPACING = 73f;

    static CubeDataFixCategoryToggleSetup()
    {
    ...
    {
        CubeDataFixManager mgr = Object.FindFirstObjectByType<CubeDataFixManager>(FindObjectsInactive.Include);
        if (mgr == null) return;

        SetupCategoryToggle(mgr);
        SetupPhotoSourceDialog(mgr);
        SetupContinueCaptureDialog(mgr);
    }

    // ============================================================
    // CategoryToggle 자동 생성/연결
    // ============================================================
    private static void SetupCategoryToggle(CubeDataFixManager mgr)
    {
    ...
    // ============================================================
    // PhotoSourceDialog 자동 연결 (기존 것 찾아서 연결)
    // ============================================================
    private static void SetupPhotoSourceDialog(CubeDataFixManager mgr)
    {
        SerializedObject so = new SerializedObject(mgr);
        SerializedProperty dialogProp = so.FindProperty("photoSourceDialog");
        if (dialogProp == null || dialogProp.objectReferenceValue != null) return;

        // 씬에서 PhotoSourceDialog 찾기 (CubeUploadManager와 공유)
        PhotoSourceDialog dialog = Object.FindFirstObjectByType<PhotoSourceDialog>(FindObjectsInactive.Include);
        if (dialog != null)
        {
            dialogProp.objectReferenceValue = dialog;
            so.ApplyModifiedPropertiesWithoutUndo();
            EditorUtility.SetDirty(mgr);
        }
    }
    ...
    // ============================================================
    // ContinueCaptureDialog 자동 연결 (기존 것 찾아서 연결)
    // ============================================================
    private static void SetupContinueCaptureDialog(CubeDataFixManager mgr)
    {
        SerializedObject so = new SerializedObject(mgr);
        SerializedProperty dialogProp = so.FindProperty("continueCaptureDialog");
        if (dialogProp == null || dialogProp.objectReferenceValue != null) return;

        ContinueCaptureDialog dialog = Object.FindFirstObjectByType<ContinueCaptureDialog>(FindObjectsInactive.Include);
        if (dialog != null)
        {
            dialogProp.objectReferenceValue = dialog;
            so.ApplyModifiedPropertiesWithoutUndo();
